...
- **다양성 필수**: ${recipesToGenerate}개의 음식은 **서로 다른 국가/지역의 요리**로 구성하세요. 한식만 나열하지 말고 일식, 중식, 양식, 동남아식, 멕시칸 등 **최소 3개 이상의 요리 문화권**에서 선택하세요.
  - 예시: 주재료가 '닭고기'라면 → 닭갈비(한식), 데리야키 치킨(일식), 치킨 까르보나라(이탈리아), 타코스(멕시코), 팟타이(태국) 등으로 다양화
...
- **요리 문화권 다양성 (필수)**: ${recipesToGenerate}개의 레시피는 **서로 다른 국가/지역의 요리**로 구성하세요. 한식에만 집중하지 말고, 최소 3개 이상의 요리 문화권(예: 한식, 일식, 중식, 양식, 동남아, 멕시칸 등)에서 선택하세요.
- **조리 방식 다양성**: 끓이기, 굽기, 볶기, 찌기, 튀기기 등 다양한 조리 방식을 사용하세요.
