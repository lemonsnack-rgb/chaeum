...
export async function analyzeInventory(ingredients: string[]): Promise<any> {
  ...
  const prompt = `🚨 가장 중요한 주의사항 (반드시 읽으세요):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"초코파이", "애플파이", "치즈파이" 같은 과자/디저트를
절대로 "파(채소)"로 분류하지 마세요!
이들은 100% 가공식품입니다!
...
다음은 사용자의 냉장고에 있는 재료 목록입니다:
${ingredients.join(', ')}

⚠️ 위 목록에 "초코파이", "애플파이" 등이 있다면 반드시 가공식품으로 분류하세요!

## 카테고리 분류 규칙

### 1. 육류
- 신선한 고기: 소고기, 돼지고기, 닭고기, 양고기

### 2. 채소
- 신선한 채소: 당근, 양파, 감자, 배추, 대파, 쪽파, 상추
- ⚠️ "대파", "쪽파"만 채소입니다
- ❌ "초코파이", "애플파이"는 절대 채소 아님

### 3. 수산물
- 신선한 해산물: 생선, 오징어, 새우, 조개

### 4. 양념
- 조미료: 소금, 간장, 고추장, 된장, 설탕, 식초, 참기름, 마늘, 생강

### 5. 가공식품 ⭐
- 과자류: 초코파이, 애플파이, 비스킷, 쿠키
- 인스턴트: 라면, 통조림
- 가공육: 햄, 소시지
- 기타: 빵, 케이크, 치즈, 음료수
- ⚠️ 이름에 "파이"가 들어간 디저트는 무조건 여기!
